...
export default function TodayTab({ onNavigate, goalMode: propGoalMode }: {
  ...
  // A user whose target weight is at or above their current weight is NOT in a
  // fat-loss scenario (e.g. an older adult maintaining at 60kg with a 60kg
  // target). The header must not call this "Fat Loss" — fall back to a neutral
  // Maintenance label when there's no actual deficit goal. Guards the case
  // where goalMode wasn't explicitly set to maintenance/geriatric.
  const targetWeight = Number(goals.targetWeightKg) || 0
  const notLosingWeight = targetWeight > 0 && targetWeight >= currentWeight
